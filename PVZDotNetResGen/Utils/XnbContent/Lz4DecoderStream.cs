...
namespace PVZDotNetResGen.Utils.XnbContent
{
	internal class Lz4DecoderStream(Stream input, long inputLength = long.MaxValue) : Stream
	{
		...
		private long inputLength = inputLength;
		...
		private int decodeBufferPos = 0, inBufPos = DecBufLen, inBufEnd = DecBufLen;

		//we keep track of which phase we're in so that we can jump right back
		//into the correct part of decoding

		private DecodePhase phase = DecodePhase.ReadToken;
		...
		private int litLen = 0, matLen = 0, matDst = 0;
